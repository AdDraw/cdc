...
path2jsons=$repo_path/syn/json_files
path2logs=$repo_path/syn/logs
...
if [[ -e ~/.yosys_show.dot ]]; then
  ...
  if [[ ! -d $repo_path/syn/dot_files ]]; then
    #statements
    mkdir $repo_path/syn/dot_files
  fi
  cp ~/.yosys_show.dot $repo_path/syn/dot_files/.$script_file_edit.dot
fi
